...
class AppointmentHelper {
  ...
  static DateTime getMonthEndDate(DateTime date) {
    final dynamic endDate = addDays(getNextMonthDate(date), -1);
    late final DateTime monthEndDate;
  ...
  static DateTime addDaysWithTime(
    ...
    final dynamic newDateValue = addDays(date, days);
    late final DateTime newDate;
    if (newDateValue is DateTime) {
            ...
            if (_isAppointmentInVisibleDateRange(
              ...
              for (int i = 0; i < 2; i++) {
                ...
                if (i == 0) {
                  spannedAppointment.actualStartTime =
                      appointment.actualStartTime;
                  final dynamic addedDate = addDays(startDate, -1);
              ...
              for (int i = 0; i < 3; i++) {
                ...
                if (i == 0) {
                  ...
                  final dynamic addedDate = addDays(endDate, 1);
                  late final DateTime date;
                  if (addedDate is DateTime) {
                    date = addedDate;
                  }
  ...
  static CalendarAppointment _cloneRecurrenceAppointment(
    ...
    final int minutes = appointment.actualEndTime
        .difference(appointment.actualStartTime)
        .inMinutes;
    final dynamic actualEndTime = addDuration(
        occurrenceAppointment.actualStartTime, Duration(minutes: minutes));
    if (actualEndTime is DateTime) {
      occurrenceAppointment.actualEndTime = actualEndTime;
    }
  ...
  static void _getRecurrenceAppointments(
    ...
    if (recurrenceRule.contains('UNTIL')) {
      ...
      final dynamic modifiedEndDate = addDuration(endDate,

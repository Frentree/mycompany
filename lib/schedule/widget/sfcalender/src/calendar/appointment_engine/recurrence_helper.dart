...
class RecurrenceHelper {
  ...
  static bool _isRecurrenceInBetweenSpecificRange(DateTime appointmentDate,
    ...
    final dynamic appEndDate = addDuration(appointmentDate, duration);
    late final DateTime appointmentEndDate;
    if (appEndDate is DateTime) {
      appointmentEndDate = appEndDate;
    }
      ...
      if (daily == 'DAILY') {
        ...
        while (i < DateTime.daysPerWeek && weeklyByDayPos != -1) {
          ...
          addDate = addDate.weekday == 6
              ? addDays(addDate,
              ((recProp.interval - 1) * DateTime.daysPerWeek) + 1)
          // ignore: avoid_as
          as DateTime
          // ignore: avoid_as
              : addDays(addDate, 1) as DateTime;
          i = i + 1;
        }
      } else if (monthly == 'MONTHLY') {
  ...
  static String _getRRuleForMonthly(
    ...
    if ((recCount > 0 &&
      ...
      if (recurrenceProperties.week == 0) {
        ...
        final dynamic modifiedFirstDate =
        addDays(DateTime.now(), -(DateTime.now().weekday - 1));
        late final DateTime firstDate;
        if (modifiedFirstDate is DateTime) {
          firstDate = modifiedFirstDate;
        }

...
class _SfCalendarState extends State<SfCalendar>
  ...
  void initState() {
    ...
    final dynamic validDate = getValidDate(widget.minDate, widget.maxDate,
        _controller.displayDate ?? widget.initialDisplayDate);
    if (validDate is DateTime) {
  ...
  void didUpdateWidget(SfCalendar oldWidget) {
    ...
    if ((oldWidget.minDate != widget.minDate) ||
      ...
      validDate = getValidDate(widget.minDate, widget.maxDate, _currentDate);
      if (validDate is DateTime) {
        _currentDate = validDate;
      }
      ...
      if (_view == CalendarView.schedule) {
        ...
        if (widget.loadMoreWidgetBuilder != null &&
            _scheduleMinDate != null &&
            _scheduleMaxDate != null) {
          final dynamic validMinDate =
          getValidDate(widget.minDate, widget.maxDate, _scheduleMinDate);
          final dynamic validMaxDate =
          getValidDate(widget.minDate, widget.maxDate, _scheduleMaxDate);
          if (validMinDate is DateTime) {
            _scheduleMinDate = validMinDate;
          }
          if (validMaxDate is DateTime) {
            _scheduleMaxDate = validMaxDate;
          }
        }
      }
    }
      ...
      for (int i = 0; i < _forwardWidgetHeights.length; i++) {
        ...
        if (scrolledPosition >= widgetPosition &&
          ...
          if (interSectionPoint != -1 &&
            ...
            final dynamic addedDate = addDays(date, 6);
            if (addedDate is DateTime) {
              date = addedDate;
            }
          }

          final dynamic validDate =
          getValidDate(widget.minDate, widget.maxDate, date);
          late final DateTime currentViewDate;
          if (validDate is DateTime) {
            currentViewDate = validDate;
          }
  ...
  void _calendarValueChangedListener(String property) {
    ...
    if (property == 'selectedDate') {
      ...
      setState(() {
        final CalendarView oldView = _view;
        _view = _controller.view!;
        final dynamic validDate = getValidDate(
            widget.minDate, widget.maxDate, _updateCurrentDate(oldView));
        if (validDate is DateTime) {
          _currentDate = validDate;
        }
  ...
  DateTime _getMaxAppointmentDate(
    ...
    for (int j = 0; j < appointments.length; j++) {
      ...
      for (int k = recursiveDates.length - 1; k >= 0; k--) {
        ...
        if (!isExceptionDate) {
          final dynamic modifiedEndDate = addDuration(
              recurrenceDate,
              appointment.actualEndTime
                  .difference(appointment.actualStartTime));
          late final DateTime recurrenceEndDate;
          if (modifiedEndDate is DateTime) {
            recurrenceEndDate = modifiedEndDate;
          }
          if (recurrenceEndDate.isAfter(currentMaxDate)) {
            currentMaxDate = recurrenceEndDate;
            break;
  ...
  Map<DateTime, List<CalendarAppointment>> _getAppointmentCollectionOnDateBasis(
    ...
    while (startDate.isBefore(endDate) || isSameDate(endDate, startDate)) {
      ...
      final dynamic modifiedStartDate = addDays(startDate, 1);
      if (modifiedStartDate is DateTime) {
  ...
  Widget? _getItem(BuildContext context, int index, bool isRTL) {
    ...
    if (isLoadMore) {
      ...
      final dynamic modifiedMinDate =
      addDays(_minDate, -(_minDate!.weekday % DateTime.daysPerWeek));
      late final DateTime viewMinDate;
      if (modifiedMinDate is DateTime) {
        viewMinDate = modifiedMinDate;
      }
      ...
      if (index > 0) {
        ...
        if (_nextDates.isNotEmpty && index > _nextDates.length - 20) {
          ...
          while (count < 20) {
            for (int i = 1; i <= 100; i++) {
              ...
              late final DateTime updateDate;
              ...
              final dynamic modifiedWeekEndDate = addDays(updateDate, 6);
              late final DateTime weekEndDate;
              if (modifiedWeekEndDate is DateTime) {
                weekEndDate = modifiedWeekEndDate;
              }
            ...
            final dynamic modifiedDate = addDays(date, 700);
            if (modifiedDate is DateTime) {
              date = modifiedDate;
            }
          }
        }
        ...
        if (_previousDates.isNotEmpty && -index > _previousDates.length - 20) {
          ...
          while (count < 20) {
            for (int i = 1; i <= 100; i++) {
              dynamic modifiedDate = addDays(date, -i * DateTime.daysPerWeek);
              ...
              modifiedDate = addDays(updatedDate, 6);
              late final DateTime weekEndDate;
              if (modifiedDate is DateTime) {
                weekEndDate = modifiedDate;
              }
    ...
    // ignore: avoid_as
        ? addDays(startDate, -DateTime.daysPerWeek) as DateTime
        : _previousDates[0]
        : (index > 0
        ? _nextDates[index - 1]
    ...
    final dynamic previousEndDate = addDays(prevDate, 6);
    late final DateTime prevEndDate;
    if (previousEndDate is DateTime) {
      prevEndDate = previousEndDate;
    }
    final dynamic modifiedEndDate = addDays(startDate, 6);
  ...
  Widget addAgenda(double height, bool isRTL) {
    ...
    late final DateTime scheduleDisplayDate;
    if (displayDate is DateTime) {
      scheduleDisplayDate = displayDate;
    }
    ...
    if (_previousDates.isEmpty) {
      ...
      DateTime date = _nextDates.isNotEmpty
          ? _nextDates[0]
      // ignore: avoid_as
          : addDays(scheduleDisplayDate, value) as DateTime;
      int count = 0;
      ...
      while (count < 50) {
        for (int i = 1; i <= 100; i++) {
          final dynamic modifiedDate = addDays(date, -i * DateTime.daysPerWeek);
          late final DateTime updatedDate;
          if (modifiedDate is DateTime) {
            updatedDate = modifiedDate;
          }
          ...
          final dynamic modifiedWeekEndDate = addDays(updatedDate, 6);
          late final DateTime weekEndDate;
          if (modifiedWeekEndDate is DateTime) {
            weekEndDate = modifiedWeekEndDate;
          }
    ...
    if (_nextDates.isEmpty) {
      dynamic modifiedDate = addDays(scheduleDisplayDate, value);
      ...
      while (count < 50) {
        for (int i = 0; i < 100; i++) {
          dynamic updateDate = addDays(date, i * DateTime.daysPerWeek);
          late final DateTime updatedDate;
          if (updateDate is DateTime) {
          ...
          updateDate = addDays(updatedDate, 6);
          late final DateTime weekEndDate;
          if (updateDate is DateTime) {
            weekEndDate = updateDate;
          }
    ...
    if (_nextDates.length < 10 && _previousDates.isNotEmpty) {
      ...
      dynamic modifiedEndDate = addDays(_nextDates[_nextDates.length - 1], 6);
      late DateTime viewEndDate;
      if (modifiedEndDate is DateTime) {
        viewEndDate = modifiedEndDate;
      }
      List<CalendarAppointment> appointmentCollection =
      AppointmentHelper.getVisibleAppointments(
          viewStartDate,
      ...
      if (_useMobilePlatformUI) {
        final dynamic modifiedPreviousDate = addDays(viewStartDate, -1);
        late DateTime previousDate;
        if (modifiedPreviousDate is DateTime) {
          previousDate = modifiedPreviousDate;
        }
        for (int i = 0; i < _nextDates.length; i++) {
      ...
      while (totalHeight < height &&
        ...
        if (totalHeight != 0) {
          ...
          modifiedEndDate = addDays(currentDate, 6);
          if (modifiedEndDate is DateTime) {
            viewEndDate = modifiedEndDate;
          }
    ...
    if (_nextDates.isNotEmpty &&
      ...
      dynamic modifiedEndDate = addDays(viewStartDate, 6);
      late final DateTime viewEndDate;
      if (modifiedEndDate is DateTime) {
        viewEndDate = modifiedEndDate;
      }
      if (viewStartDate.isBefore(scheduleDisplayDate) &&
        ...
        modifiedEndDate = addDays(scheduleDisplayDate, -1);
        late final DateTime viewEndDate;
        if (modifiedEndDate is DateTime) {
          viewEndDate = modifiedEndDate;
        }
  ...
  Widget addAgendaWithLoadMore(double height, bool isRTL) {
    ...
    if (_previousDates.isEmpty ||
      ...
      while (count < 50) {
        for (int i = 1; i <= 100; i++) {
          final dynamic updateDate = addDays(date, -i * DateTime.daysPerWeek);
          late final DateTime updatedDate;
          if (updateDate is DateTime) {
            updatedDate = updateDate;
          }
          ...
          final dynamic modWeekEndDate = addDays(updatedDate, 6);
          late final DateTime weekEndDate;
          if (modWeekEndDate is DateTime) {
            weekEndDate = modWeekEndDate;
          }
    ...
    final dynamic modifiedMaxDate = addDays(_maxDate,
        (DateTime.daysPerWeek - _maxDate!.weekday) % DateTime.daysPerWeek);
    late final DateTime viewMaxDate;
    if (modifiedMaxDate is DateTime) {
      viewMaxDate = modifiedMaxDate;
    }
    if (_nextDates.isEmpty ||
      ...
      while (count < 50) {
        for (int i = 0; i < 100; i++) {
          ...
          final dynamic modWeekEndDDate = addDays(updatedDate, 6);
          late final DateTime weekEndDate;
          if (modWeekEndDDate is DateTime) {
            weekEndDate = modWeekEndDDate;
          }
        ...
        final dynamic modifiedEndDate = addDays(date, 700);
        if (modifiedEndDate is DateTime) {
    ...
    if (_nextDates.length < 10 && _previousDates.isNotEmpty) {
      ...
      /// appointment fills the view port.
      DateTime viewStartDate = _nextDates[0];
      modifiedMinDate = addDays(_nextDates[_nextDates.length - 1], 6);
      late DateTime viewEndDate;
      if (modifiedMinDate is DateTime) {
        viewEndDate = modifiedMinDate;
      }
      List<CalendarAppointment> appointmentCollection =
      AppointmentHelper.getVisibleAppointments(
          viewStartDate,
      ...
      double labelHeight = 0;
      if (_useMobilePlatformUI) {
        final dynamic modPreviousDate = addDays(viewStartDate, -1);
        late DateTime previousDate;
        if (modPreviousDate is DateTime) {
          previousDate = modPreviousDate;
        }
        for (int i = 0; i < _nextDates.length; i++) {
          final DateTime nextDate = _nextDates[i];
          if (previousDate.month != nextDate.month) {
      ...
      while (totalHeight < height &&
        ...
        if (totalHeight != 0) {
          ...
          viewStartDate = currentDate;
          final dynamic modifiedEndDate = addDays(currentDate, 6);
          if (modifiedEndDate is DateTime) {
            viewEndDate = modifiedEndDate;
          }
      ...
      if (_nextDates.isNotEmpty && isNewDatesAdded) {
        ...
        _headerUpdateNotifier.value = _useMobilePlatformUI
            ? date
        // ignore: avoid_as
            : getValidDate(_minDate, _maxDate, date) as DateTime;
      }
    }
    ...
    if (_nextDates.isNotEmpty &&
      ...
      while (isSameOrBeforeDate(_maxDate, viewStartDate)) {
        ...
        final dynamic modifiedViewStartDate =
        addDays(viewStartDate, DateTime.daysPerWeek);
        if (modifiedViewStartDate is DateTime) {
          viewStartDate = modifiedViewStartDate;
        }
      }
      ...
      for (int i = 0; i < _nextDates.length; i++) {
        ...
        final dynamic endDate = addDays(visibleStartDate, DateTime.daysPerWeek);
        late final DateTime visibleEndDate;
    ...
    if (isNeedViewSwitchOption) {
      ...
      if (useMobilePlatformUI) {
        ...
        calendarViewIcon = _getCalendarViewWidget(
            ...
            headerTextColor,
            widget.view,
            !widget.isMobilePlatform || widget.viewChangeNotifier.value,
            highlightColor,
